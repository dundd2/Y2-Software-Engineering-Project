...
class GameRenderer:
    ...
    def draw(self):
        ...
        if self.game.development_mode and self.game.state == "DEVELOPMENT":
            if (
                not any(player.is_moving for player in self.game.players)
                ...
                if not current_player.get("is_ai", False):
                    if (
                        not hasattr(self.game, "dev_notification")
                        or self.game.dev_notification is None
                    ):
    ...
    def draw_development_ui(self, property_data):
        ...
        pygame.draw.rect(
            self.screen,
            color,
            self.game.development_buttons["upgrade"],
            border_radius=5,
        )

        btn_text = self.small_font.render(upgrade_text, True, WHITE)
        text_rect = btn_text.get_rect(
            center=self.game.development_buttons["upgrade"].center
        )
        self.screen.blit(btn_text, text_rect)
        y_offset += button_height + button_margin

        self.game.development_buttons["auction"] = pygame.Rect(
            card_x + 20, y_offset, button_width, button_height
        )
        hover = self.game.development_buttons["auction"].collidepoint(mouse_pos)
        pygame.draw.rect(
            self.screen,
            BUTTON_HOVER if hover else ACCENT_COLOR,
            self.game.development_buttons["auction"],
            border_radius=5,
        )

        btn_text = self.small_font.render("Auction to Player", True, WHITE)
        text_rect = btn_text.get_rect(
            center=self.game.development_buttons["auction"].center
        )
        self.screen.blit(btn_text, text_rect)
        y_offset += button_height + button_margin
        ...
        self.game.development_buttons["mortgage"] = pygame.Rect(
            card_x + 20, y_offset, button_width, button_height
        )

        if is_mortgaged:
            hover = (
                self.game.development_buttons["mortgage"].collidepoint(mouse_pos)
                and can_unmortgage
            )
        else:
            hover = (
        ...
        pygame.draw.rect(
            self.screen,
            color,
            self.game.development_buttons["mortgage"],
            border_radius=5,
        )

        btn_text = self.small_font.render(mortgage_text, True, WHITE)
        text_rect = btn_text.get_rect(
            center=self.game.development_buttons["mortgage"].center
        )
        self.screen.blit(btn_text, text_rect)
        y_offset += button_height + button_margin
        ...
        self.game.development_buttons["sell"] = pygame.Rect(
            card_x + 20, y_offset, button_width, button_height
        )
        hover = (
            self.game.development_buttons["sell"].collidepoint(mouse_pos) and can_sell
        )
        color = BUTTON_HOVER if hover else (ACCENT_COLOR if can_sell else GRAY)
        pygame.draw.rect(
            self.screen, color, self.game.development_buttons["sell"], border_radius=5
        )

        btn_text = self.small_font.render(sell_text, True, WHITE)
        text_rect = btn_text.get_rect(
            center=self.game.development_buttons["sell"].center
        )
        self.screen.blit(btn_text, text_rect)
        y_offset += button_height + button_margin

        self.game.development_buttons["close"] = pygame.Rect(
            card_x + 20, y_offset, button_width, button_height
        )
        hover = self.game.development_buttons["close"].collidepoint(mouse_pos)
        pygame.draw.rect(
            self.screen,
            BUTTON_HOVER if hover else ERROR_COLOR,
            self.game.development_buttons["close"],
            border_radius=5,
        )

        btn_text = self.small_font.render("Close", True, WHITE)
        text_rect = btn_text.get_rect(
            center=self.game.development_buttons["close"].center
        )
        self.screen.blit(btn_text, text_rect)
