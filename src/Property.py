...
class Property:
    ...
    def calculate_rent(self, dice_roll=None, properties=None):
        ...
        if self.is_station:
            ...
            station_count = sum(1 for p in properties if p.is_station and p.owner == self.owner)
            return self.base_rent * (2 ** (station_count - 1))
            
        elif self.is_utility:
            if not properties or not dice_roll:
                return 0
            utility_count = sum(1 for p in properties if p.is_utility and p.owner == self.owner)
            ...
            if self.has_hotel:
                ...
                return self.house_costs[house_index] if self.house_costs else self.base_rent * (self.houses + 1)
