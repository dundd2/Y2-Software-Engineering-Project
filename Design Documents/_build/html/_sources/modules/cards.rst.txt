...
.. uml::
   ...
   state Deck {
     Ready : Draw pile has cards
     DrawEmpty : Draw pile empty
     NoCardsLeft : Both piles empty (Error state or needs re-init)
...
.. uml::
   ...
   activate Deck
   Deck -> Deck : Search (self.cards + self.discard_pile) for "jail free" card
   alt Found jail_card instance
      note right: Uses the existing card instance found in draw or discard pile.
   else Not found
      Deck -> Data : Get "jail free" text/action for card_type
      Data --> Deck : text, action
      Deck -> CardClass : create Card(text, action, card_type)
      CardClass --> Deck : jail_card
      note right: Newly created instance if none exists in the deck/discard.
   end
   Deck -> Deck : return_card(jail_card, to_bottom=True)
   activate Deck # return_card call
   Deck -> Deck : self.discard_pile.append(jail_card)
   deactivate Deck # return_card call
   Deck --> Caller
   deactivate Deck
   @enduml
