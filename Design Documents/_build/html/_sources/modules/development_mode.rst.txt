...
.. uml::
    ...
    note over Caller: e.g., AI Controller or Game Loop
    participant "dev_mode:DevelopmentMode" as DevMode
    participant "game:Game" as Game
    participant "logic:GameLogic" as Logic
    ...
    activate DevMode
    DevMode -> Game : lap_count.get(player_name)
    Game --> DevMode : lap_count
    alt lap_count < 1
        DevMode --> Caller : return False
    else lap_count >= 1
        DevMode -> Logic : properties.values()
        Logic --> DevMode : all_properties
        DevMode -> DevMode : Filter owned_properties
        alt no owned_properties
            DevMode --> Caller : return False
        else has owned_properties
            loop for prop in owned_properties
                DevMode -> Logic : can_build_house(prop, player)
                Logic --> DevMode : can_build_h, _
                DevMode -> Logic : can_build_hotel(prop, player)
                Logic --> DevMode : can_build_ht, _
                DevMode -> DevMode : Check mortgage/unmortgage/sell conditions
                if can_build_h or can_build_ht or can_mortgage or can_unmortgage or can_sell
                    DevMode --> Caller : return True
                    break loop
                endif
            end loop
            note over DevMode: If loop finishes without returning True
            DevMode --> Caller : return False
        end
    end
    deactivate DevMode
...
.. uml::
    :caption: Sequence Diagram: Mortgage/Unmortgage Action (UI Click)
    ...
    note over Player, Board: Precondition: Development mode active, property selected
    Player -> DevMode : handle_click(pos_on_mortgage_button)
    activate DevMode
    DevMode -> DevMode : Check button collision (buttons["mortgage"])
    alt "mortgage" button clicked
        DevMode -> Game : selected_property.get("is_mortgaged")
        Game --> DevMode : is_mortgaged
        if is_mortgaged then
            DevMode -> Logic : unmortgage_property(prop, player)
            activate Logic
            Logic -> Logic : Check funds, update player money, update prop state
            opt Action successful
                Logic --> DevMode : return True
                DevMode -> Board : add_message("... unmortgaged ...")
            else Action failed
                Logic --> DevMode : return False
            end
            deactivate Logic
        else not is_mortgaged
            DevMode -> Logic : mortgage_property(prop, player)
            activate Logic
            Logic -> Logic : Check buildings==0, update player money, update prop state
            opt Action successful
                Logic --> DevMode : return True
        ...
        opt Action successful (result == True)
            DevMode -> Board : update_ownership(logic.properties)
        end
        DevMode --> Player : return False (stay in UI)
    else Other button or no button clicked
        note right: Handle other actions or return
        DevMode --> Player : return False or None
    end
    deactivate DevMode
